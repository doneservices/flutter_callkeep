...
class CallKeepPlugin(private val activity: Activity, private val applicationContext: Context, private val channel: MethodChannel) : MethodChannel.MethodCallHandler, PluginRegistry.RequestPermissionsResultListener {
    companion object {
        @JvmStatic
        fun registerWith(registrar: PluginRegistry.Registrar) {
            val channel = MethodChannel(registrar.messenger(), "co.doneservices/callkeep")
            val plugin = CallKeepPlugin(registrar.activity(), registrar.context(), channel)
    ...
    private fun checkPhoneAccountPermission(optionalPermissions: Array<String>, result: MethodChannel.Result) {
        ...
        if (!this.hasPermissions()) {
            ActivityCompat.requestPermissions(currentActivity, allPermissions, REQUEST_READ_PHONE_STATE)
            return
        }
    ...
    private fun hasPermissions(): Boolean {
        ...
        for (permission in requiredPermissions) {
            val permissionCheck = ContextCompat.checkSelfPermission(currentActivity, permission)
